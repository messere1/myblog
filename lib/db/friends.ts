...
export async function getPublicFriends(): Promise<FriendRecord[]> {
  const result = await db.execute(`SELECT * FROM friends WHERE deleted_at IS NULL ORDER BY sort_order ASC, created_at DESC`);
  return result.rows as unknown as FriendRecord[];
}

export async function getAdminFriends(): Promise<FriendRecord[]> {
  const result = await db.execute(`SELECT * FROM friends WHERE deleted_at IS NULL ORDER BY sort_order ASC, updated_at DESC`);
  return result.rows as unknown as FriendRecord[];
}
...
export async function createFriend(input: { name: string; url: string; avatar_url?: string | null; description?: string | null; site_title?: string | null; sort_order?: number }): Promise<FriendRecord> {
  ...
  await db.execute({
    ...
    args: [id, input.name, input.url, input.avatar_url || null, input.description || null, input.site_title || null, 'approved', input.sort_order || 0, now, now, now],
  });
  return getFriendById(id) as Promise<FriendRecord>;
}

export async function updateFriend(id: string, input: Partial<{ name: string; url: string; avatar_url: string | null; description: string | null; site_title: string | null; sort_order: number }>): Promise<void> {
  const now = new Date().toISOString();
  const fields: string[] = [];
