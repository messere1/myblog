...
const CloudPlayerLyric = memo(function CloudPlayerLyric() {
  ...
  return (
    <div className="relative z-10 mb-2 h-6 overflow-hidden">
      <p className="truncate text-xs font-bold text-sky-600 dark:text-sky-400">{currentLyric}</p>
    </div>
  );
});
...
export default function CloudPlayer() {
      ...
      >
        <div className="mb-6 mt-2 flex items-center gap-5">
          ...
          <div className="flex-1 space-y-3">
            <div className="h-4 w-24 rounded-sm bg-sky-500/20" />
        ...
        <div className="h-3 w-4/5 rounded-full bg-sky-500/15" />
        <div className="mt-auto pt-5">
          ...
          <div className="flex items-center justify-center gap-5">
            ...
            <div className="h-12 w-12 rounded-full border-2 border-white/60 bg-sky-500/35" />
            <div className="h-6 w-6 rounded-full bg-stone-200/60 dark:bg-stone-700/50" />
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className={`absolute -right-20 -top-20 h-48 w-48 rounded-full bg-sky-300/22 blur-[50px] transition-opacity duration-1000 ${isPlaying ? 'opacity-100' : 'opacity-35'}`}></div>
      ...
      <div className="flex items-center gap-5 relative z-10 mb-6 mt-2">
        ...
        <div className="flex-col overflow-hidden w-full">
          <span className="rounded-sm bg-white/55 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-sky-600 shadow-sm dark:bg-stone-900/50 dark:text-sky-400">Cloud Music</span>
      ...
      <div className="relative z-10 mt-auto" onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-2">
          <button onClick={safeTogglePlayMode} className="relative z-20 p-1 text-stone-700 transition-colors hover:text-sky-600 dark:text-stone-300 dark:hover:text-sky-400" title={playMode === 'loop' ? '顺序播放' : playMode === 'single' ? '单曲循环' : '随机播放'}>
            {getModeIcon()}
          </button>

          <div className="flex items-center gap-5">
            <button onClick={safePrevSong} className="relative z-20 text-stone-700 transition-colors hover:text-sky-600 dark:text-stone-300 dark:hover:text-sky-400">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
            </button>
            <button onClick={safeTogglePlay} className="relative z-20 flex h-12 w-12 items-center justify-center rounded-full border-2 border-white/60 bg-sky-500 text-white shadow-lg shadow-sky-600/20 transition-all hover:scale-110 hover:bg-sky-600 dark:border-stone-600">
            ...
            <button onClick={safeNextSong} className="relative z-20 text-stone-700 transition-colors hover:text-sky-600 dark:text-stone-300 dark:hover:text-sky-400">
    ...
    <div className="flex items-center relative z-20" onClick={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()}>
      {showVolume && (
        <div className="absolute bottom-full right-0 mb-2 w-24 rounded-full border border-white/40 bg-white/80 px-3 py-2 shadow-lg dark:border-white/10 dark:bg-stone-800/80" onMouseLeave={() => setShowVolume(false)}>
          <input type="range" min="0" max="1" step="0.01" value={isMuted ? 0 : volume} onChange={onSetVolume} className="h-1 w-full cursor-pointer appearance-none rounded-full" style={{ background: `linear-gradient(to right, #38bdf8 ${(isMuted ? 0 : volume) * 100}%, rgba(2,132,199,0.22) ${(isMuted ? 0 : volume) * 100}%)` }} />
        </div>
      )}
      <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowVolume(!showVolume); }} onDoubleClick={(e) => { e.preventDefault(); e.stopPropagation(); onToggleMute(); }} className="p-1 text-stone-700 transition-colors hover:text-sky-600 dark:text-stone-300 dark:hover:text-sky-400">
        {isMuted || volume === 0
